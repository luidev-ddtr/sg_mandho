...
function EditCustomerModal({ id_user, onClose, onUpdate }) {
  ...
  // Opciones para el dropdown de manzana/fracción
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      ...
      try {
        ...
        if (response?.success) {
          ...
          // Establecer valores iniciales SOLO para campos editables
          reset({
    ...
    console.log("Laamando desde editar");
    fetchCustomer();
  }, [id_user, reset]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      // Payload solo con campos editables
      const payload = {
      ...
      if (response && response.success) {
        if (onUpdate) {
          onUpdate(response.data); // Asumiendo que el backend devuelve el usuario actualizado
          }
        setSuccess(true);
        setTimeout(() => {
          onClose();
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-4">
              ...
              {/* NUEVA SECCIÓN: Estado de la Persona */}
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  className={`form-select w-full rounded-lg ${errors.estadoPersona ? 'border-red-500' : 'border-gray-300'} dark:bg-gray-700 dark:border-gray-600 dark:text-white`}
                  disabled={isSubmitting || success}
                >
                ...
                {errors.estadoPersona && (
                  <p className="mt-1 text-sm text-red-500 dark:text-red-400">{errors.estadoPersona.message}</p>
                )}
              </div>

            {error && (
              <div className="mt-4 p-3 bg-red-100 dark:bg-red-900/30 border border-red-400 dark:border-red-700 text-red-700 dark:text-red-100 rounded-lg">
                {error}
              </div>
            )}
